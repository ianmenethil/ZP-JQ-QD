...
/**
 * Safely stringifies a value with circular reference handling
...
/**
 * Formats console arguments with color styling
 * @param args - Arguments to format
 * @param defaultColor - CSS color string for styling
 * @returns Formatted arguments array with color codes
 */
function formatArgs(args: readonly unknown[], defaultColor: string): unknown[] {
	...
	if (typeof head === 'string' && rest.length > 0) {
		const printable = rest.map((a) =>
			typeof a === 'object' && a !== null && !(a instanceof Error) ? safeStringify(a) : a
		);
...
import { EventListenerError } from './buttonState.logic.ts';
	...
	return function <T extends (...args: unknown[]) => unknown>(
		...
		descriptor.value = function (this: unknown, ...args: unknown[]): unknown {
			try {
				...
				applogger.error(`[${fnName}] Error:`, error);
				throw new EventListenerError(
					`Failed to initialize ${fnName}`,
					fnName,

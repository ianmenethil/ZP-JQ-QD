...
export function showCustomModal(
    ...
    `;
        ...
        throw new ModalCreationError('Failed to show custom modal', error instanceof Error ? error : undefined);
    }
}
    ...
    return new Promise((resolve) => {
        try {
            ...
            if (!container) {
                ...
                container.className = `position-fixed ${position === 'top-right' ? 'top-0 end-0' : 
                    position === 'top-left' ? 'top-0 start-0' :
                    position === 'bottom-right' ? 'bottom-0 end-0' :
                    position === 'bottom-left' ? 'bottom-0 start-0' :
                    'top-0 start-50 translate-middle-x'} p-3`;

 ...
 */
function extractTableData(html, tableTitle) {
	const results = [];
 ...
 */
function extractAllReturnParameters(html) {
	const results = {
		mode02: [],
		mode1: [],
		combined: [],
	...
	console.log(
		`✅ Extracted ${results.mode02.length} Mode 0&2 parameters, ${results.mode1.length} Mode 1 parameters`
	);
	return results;
}
 ...
 */
function extractCodeSamples(html) {
	const results = [];
...
function cleanupFiles() {
	const filesToRemove = [
		path.join(OUTPUT_DIR, 'zenpay-demo.html'),
		path.join(OUTPUT_DIR, 'raw-extracted-data.json'),
	];
 ...
 */
async function extractZenPayData() {
	try {
		...
		const inputParameters = extractTableData(html, 'Input Parameters');
		...
		const errorCodes = extractTableData(html, 'Error Codes');
